...
export default function Sanxingdui(): React.FC {
  ...
  return (
    <div className="w-full h-screen bg-[#000406] overflow-hidden text-white font-sans selection:bg-cyan-500/30">
      {/* 3D Canvas Layer */}
      ...
      {/* Scanlines Overlay */}
      <div className="fixed inset-0 pointer-events-none z-10 opacity-30 bg-[linear-gradient(rgba(18,16,16,0)_50%,rgba(0,255,204,0.02)_50%)] bg-[length:100%_4px]"></div>
      ...
      <header className="absolute top-0 left-0 w-full p-12 flex justify-between items-start z-20 pointer-events-none">
        <div>
            ...
            <h1 className="text-3xl font-extralight tracking-[0.5em] mb-2 uppercase">Sanxingdui</h1>
            <h2 className="text-lg font-light tracking-[0.2em] text-white/40">沉睡三千年，一醒惊天下。</h2>
        </div>
        <button
      ...
      <footer className="absolute bottom-0 left-0 w-full p-12 flex justify-between items-end z-20 pointer-events-none">
         <div className="space-y-4 pointer-events-auto">
             <button
               ...
               className="px-4 py-2 rounded-full bg-white/5 hover:bg-white/10 border border-white/10 text-xs tracking-[0.2em] uppercase text-white/60"
             >
               {gestureEnabled ? "手势控制：开启" : "手势控制：关闭"}
             </button>
         </div>
         <div className="text-right flex flex-col items-end">
             <div className="text-[8px] text-white/20 tracking-[0.3em] uppercase mb-1">Current Artifact</div>
             <div className="text-sm font-light tracking-[0.2em] text-white/80">
      ...
      <div className={`absolute right-12 top-1/2 -translate-y-1/2 w-96 bg-black/20 backdrop-blur-md border border-white/10 p-10 rounded-[2.5rem] z-30 transition-all duration-700 ${currentModelIndex === -1 ? 'opacity-0 translate-x-12' : 'opacity-100 translate-x-0'}`}>
        <div className="text-[8px] text-yellow-500 tracking-[0.5em] mb-6 uppercase">Asset Record</div>
        <h3 className="text-2xl font-light mb-6 border-b border-white/10 pb-6 tracking-wide text-white">
            {currentModelIndex >= 0 ? models[currentModelIndex].name : ''}
        </h3>
        <p className="text-xs leading-relaxed text-white/60 font-light text-justify italic">
