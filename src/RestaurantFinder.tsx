...
import { MapPin, Utensils, RefreshCw, Navigation, Compass, LocateFixed } from 'lucide-react';
...
const RestaurantFinder = () => {
  ...
  const searchRestaurants = (lat: number, lng: number) => {
    ...
    placeSearch.current.searchNearBy('', center, searchRadius, (status: string, result: any) => {
      if (status === 'complete' && result?.poiList?.pois?.length > 0) {
        pickRandomRestaurant(result.poiList.pois);
      } else if (searchRadius === 500) {
        setSearchRadius(1000);
        placeSearch.current.searchNearBy('', center, 1000, (status: string, result: any) => {
          setLoading(false);
          if (status === 'complete' && result?.poiList?.pois?.length > 0) {
            pickRandomRestaurant(result.poiList.pois);
          } else {
  ...
  const pickRandomRestaurant = (restaurants: any[]) => {
        ...
        >
          <div className="p-6 bg-gradient-to-r from-blue-600 to-indigo-600 text-white">
            ...
            <p className="mt-2 opacity-90">
              帮你解决"今天吃什么"的难题，随机推荐附近{searchRadius}米内的餐厅
            </p>
          </div>
                ...
                <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                  ...
                  <div className="mt-6 flex flex-col sm:flex-row gap-3">
                    ...
                    <motion.button
                      whileHover={{ scale: 1.02 }}
                      whileTap={{ scale: 0.98 }}
                      onClick={() => searchRestaurants(position!.lat, position!.lng)}
